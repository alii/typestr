...
export type TupleHasLength<Tuple, IfTrue = 1, IfFalse = 0> = Tuple extends [unknown, ...unknown[]]
	? IfTrue
	: IfFalse;
...
export type PartTuple<Tuple extends unknown[], Part> = Tuple extends [...infer First, Part]
	? First
	: Tuple;

export type Indices<T extends {length: number}> = Exclude<Partial<T>['length'], T['length']>;
